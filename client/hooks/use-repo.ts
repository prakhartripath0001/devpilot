...
function updateRepoInListCache(
  ...
  queryClient.setQueryData<Repository[]>(queryKeys.repo.list(), (current) => {
...
export function useRepository(repoId: string) {
  return useQuery({
    queryKey: queryKeys.repo.detail(repoId),
...
export function useStartIndexing() {
  ...
  return useMutation({
    ...
    onSuccess: (repo) => {
      queryClient.setQueryData(queryKeys.repo.detail(repo.id), repo);
      updateRepoInListCache(queryClient, repo);
      void queryClient.invalidateQueries({
        queryKey: queryKeys.repo.status(repo.id),
      });
      toast.add({
...
export function useRefreshRepos() {
  ...
  return useMutation({
    ...
    onSuccess: (repos) => {
      queryClient.setQueryData(queryKeys.repo.list(), repos);
    },
  });
}
